...
- **Aguara core v0.14.4 -> v0.17.0.** The embedded scanner now ships with multi-ecosystem threat intel (npm, PyPI, Go, crates.io, Packagist, RubyGems, Maven, NuGet) from an OSV-derived snapshot, analyzer-emitted rule metadata via the rulecatalog (jsrisk / toxicflow / pkgmeta / ci-trust / NLP / rug-pull), and sensitivity-based in-place redaction of `Finding.MatchedText`. The MCP inherits all three.
